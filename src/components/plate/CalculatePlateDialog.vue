  ...
  <q-dialog v-model="showDialog" persistent>
    <q-card style="min-width: 60vw">
      ...
      <q-card-section>
        ...
        <q-card-section>
          <div class="q-pb-sm">Select the protocol to use for calculation:</div>
          <protocol-selectable-list
...
const checkDimensions = () => {
  ...
  if (activeMeasurement.value) {
    return (
      activeMeasurement.value.rows === props.plate.rows &&
      activeMeasurement.value.columns === props.plate.columns
    );
  }
  return true;
};
