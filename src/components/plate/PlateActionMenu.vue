<template>
  <q-menu>
    <q-list>
      <!-- Validation Menu -->
      <q-item v-if="plate.calculationStatus !== 'CALCULATION_NEEDED' && plate.approvalStatus === 'APPROVAL_NOT_SET'"
              dense clickable>
        ...
        <q-menu anchor="top end" self="top start">
          <q-list>
            <q-item v-if="plate.validationStatus === 'VALIDATION_NOT_SET'" @click="validate()" dense clickable>
              <q-item-section avatar>
                <q-icon name="check_circle"/>
              </q-item-section>
              <q-item-section>Validate</q-item-section>
            </q-item>
            <q-item v-if="plate.validationStatus === 'VALIDATION_NOT_SET'" @click="invalidate()" dense clickable>
              <q-item-section avatar>
                <q-icon name="cancel"/>
              </q-item-section>
              <q-item-section>Invalidate</q-item-section>
            </q-item>
            <q-item v-if="plate.validationStatus !== 'VALIDATION_NOT_SET'" @click="resetValidation()" dense clickable>
              <q-item-section avatar>
      ...
      <!-- Approval Menu -->
      <q-item v-if="plate.validationStatus === 'VALIDATED' && plate.approvalStatus === 'APPROVAL_NOT_SET'"
              dense clickable>
        <q-item-section avatar>
          <q-icon name="outlined_flag"/>
        </q-item-section>
      ...
      <q-item dense clickable @click="openPlateInspector()">
        <q-item-section avatar>
          <q-icon name="info"/>
        </q-item-section>
...
const emit = defineEmits(['showPlateInspector'])
