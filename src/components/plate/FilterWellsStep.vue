<template>
  ...
  <div class="q-pa-xs">
    <div>
      <q-checkbox
        v-model="wellFilter.includeRejectedWells"
        label="Include rejected wells"
        dense
      />
    </div>
  </div>
...
const addSubstance = () => {
  if (substanceName.value || substanceType.value)
    wellFilter.value.substances.push({
      name: substanceName.value ?? "",
      type: substanceType.value ?? "",
    });
};
