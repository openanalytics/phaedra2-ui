    ...
    <q-card style="min-width: 50vw">
      ...
      <q-card-section>
        <div class="row q-pb-md">
          <span
            >By linking plates with a measurement it allows the user to apply
            protocol(s) with calculation features on the plate(s)</span
          >
        </div>
        ...
        <q-list dense bordered>
          <q-item v-for="plate in props.plates" :key="plate.id">
            ...
            <q-item-section
              >{{ plate.barcode }} ({{ plate.id }}) with dimensions
              {{ plate.rows }} x {{ plate.columns }}</q-item-section
            >
          </q-item>
        </q-list>
      </q-card-section>
      ...
      <q-card-actions class="text-primary" align="right">
        ...
        <q-btn
          ...
          color="primary"
          @click="doLink"
          :disable="selectedMeasurement.length == 0"
          v-close-popup
        />
      </q-card-actions>
    </q-card>
...
const filteredMeasurements = computed(() =>
  preFilterMeasurements(allMeasurements.value)
);

const loadingHandler = useLoadingHandler();
const doLink = async () => {
  await loadingHandler.handleLoadingDuring(
    plateActions.linkMeasurement(props.plates, selectedMeasurement.value[0])
  );
...
const columns = [
  ...
  },
  {
    ...
    field: "layout",
    sortable: true,
    format: (val, row) => row.rows + " x " + row.columns,
  },
  {
    name: "wellColumns",
    align: "left",
  ...
  },
  {
    name: "subWellColumns",
    align: "left",
    label: "SubWell Columns",
    field: "subWellColumns",
    sortable: true,
    format: (val) => `${val?.length || 0}`,
  },
  {
...
const checkPlateDimensions = () => {
  ...
  const countColumns = props.plates
    .map((p) => p.columns)
...
const preFilterMeasurements = (allMeasurements) => {
  ...
  return allMeasurements.filter(
    (row) =>
      row.rows === props.plates[0].rows &&
      row.columns === props.plates[0].columns
  );
};
