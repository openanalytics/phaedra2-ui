<template>
    <div :class="{ 'cursor-pointer': props.editable }" :style="{ width: '25px', border: '1px solid grey', backgroundColor: color }">
        &nbsp;
        <q-popup-proxy v-if="props.editable" cover transition-show="scale" transition-hide="scale">
            <q-color v-model="color" no-header @change="doEmitValue" />
        </q-popup-proxy>
