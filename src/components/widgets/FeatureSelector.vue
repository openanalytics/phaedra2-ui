<template>
  <div class="row no-wrap items-center">
    ...
    <div class="col-8">
      <q-select
        v-if="selectedFeatureOption === 'raw'"
      ...
      />
      <q-select
        v-if="selectedFeatureOption === 'calculated'"
...
const initFeatureOptions = () => {
  allRawFeatures.value = measurements.value
    .flatMap((measurement) =>
      measurement?.wellColumns.map((wellCol) => {
        return {
    ...
    .filter(
      (obj, index, self) =>
        index === self.findIndex((el) => el.column === obj.column)
    )
    .filter((obj, index, self) => obj.column !== "")
    .sort((f1, f2) => f1.column.localeCompare(f2.column));
  rawFeatureOptions.value = allRawFeatures.value;

  allCalculatedFeatures.value = protocols.value.flatMap((protocol) =>
    protocol.features
      .map((feature) => {
        return {
          name: `[${protocol.name}] ${feature.name}`,
...
const filterCalculatedOptions = (filter, update) => {
  update(() => {
    calculatedFeatureOptions.value = allCalculatedFeatures.value.filter(
      (cfo) => {
