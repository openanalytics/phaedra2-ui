...
export default {
  ...
  async editExperiment(experiment) {
    const response = await axios.put(
      `${apiURL}/experiments/${experiment.id}`,
      experiment
    );
    if (response.status === 200) return response.data;
  },
  ...
  async deleteExperiments(ids) {
    const response = await axios.delete(`${apiURL}/experiments`, { data: ids });
    if (response.status === 200) return response.data;
  },
  ...
  async getExperimentSummariesByProjectId(id) {
    const response = await axios.get(
      `${apiURL}/projects/${id}/experimentsummaries`
    );
