...
const onSubmit = async () => {
  ...
  reader.onload = (res) => {
    ...
    if (plateTemplate.wells) {
      newPlateTemplate.rows = plateUtils.getPlateDimensions(
        plateTemplate.wells.length()
      ).rows;
      newPlateTemplate.columns = plateUtils.getPlateDimensions(
        plateTemplate.wells.length()
      ).cols;
    }
    ...
    router.push({
      ...
      params: { plateTemplate: newPlateTemplate },
    });
