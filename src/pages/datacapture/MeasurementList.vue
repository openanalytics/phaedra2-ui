...
const columns = ref([
  ...
  },
  {
    name: "wellColumns",
    align: "left",
    label: "WellData Columns",
    field: (row) => row?.wellColumns?.length || 0,
    sortable: true,
  },
  {
    name: "subWellColumns",
    align: "left",
    label: "SubwellData Columns",
    field: (row) => row?.subWellColumns?.length || 0,
    sortable: true,
  },
  {
    name: "imageChannels",
    ...
    field: (row) => row?.imageChannels?.length || 0,
    sortable: true,
  },
]);
...
const refreshList = async () => {
  await loadingHandler.handleLoadingDuring(
    measurementStore.loadAllMeasurements(getDateRange())
  );
};
