<template>
  <q-card style="width: 1000px; max-width: 80vw;">
    <q-card-section class="oa-section">
      <div class="row align-center text-h5">
        <q-icon name="scanner" class="on-left" size="md"/>
        <span>DataCapture Job Details</span>
      </div>
    </q-card-section>
    ...
    <q-card-section class="q-pa-md q-gutter-sm">
        ...
        <div class="col-9">
          ...
          >
            <template v-slot:body-cell-eventType="props">
              <q-td :props="props">
                <q-badge :color="ColorUtils.getCaptureJobEventTypeColor(props.row.eventType)">
                  {{ props.row.eventType }}
                </q-badge>
              </q-td>
            </template>
      ...
      <div class="row">
        ...
        <div class="col-9">
          ...
          <q-btn v-if="!showConfig" label="Show" @click="showConfig=true" size="sm" color="primary"
                 icon="remove_red_eye"/>
          <q-btn v-if="showConfig" label="Hide " @click="showConfig=false" class="q-mt-sm" size="sm"
                 color="primary" icon="remove_red_eye"/>
        </div>
      </div>
    </q-card-section>
  </q-card>
